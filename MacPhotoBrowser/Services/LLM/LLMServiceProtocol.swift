...
enum TaggingPrompts {
    ...
    3. タグは3〜5個、簡潔で代表的なキーワードに厳選。
    4. 意味が重なるタグは1つにまとめる（例: "猫"と"ネコ"、"海岸"と"ビーチ"は片方だけ）。
    5. 抽象的すぎるタグ（"写真"、"画像"等）は禁止。
    6. 説明文は1〜2文で、写真の主要な内容を要約。
    7. 出力は必ず valid JSON のみ。それ以外のテキストは禁止。
    ...
    static func userPromptForOCR(_ ocrText: String) -> String {
        ...
        出力形式（JSONのみ、説明不要）:
        {"tags": ["料理", "ラーメン"], "description": "内容の要約"}
        """
    }
    ...
    ルール:
    - 写真に写っている被写体、場所、雰囲気等からタグを生成
    - タグは3〜5個に厳選し、意味が似たタグは1つにまとめる
    - 説明文は1〜2文で写真の内容を要約
    - 日本語で出力
